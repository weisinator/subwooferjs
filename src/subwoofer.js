...
function subwoofer(d, e, dc = 1) {
    ...
    this.portLength = function(){
        ...
        return 23562.5*Math.pow(circle.areaToDiameter(this.portArea),2)/(this.enclosure.vb*Math.pow(this.enclosure.fb,2))
    }
    this.frequencyResponse = function(f){
        if(this.enclosure.fb == -1){
            return this.sealedFrequencyResponse(f);
        }
        return this.portedFrequencyResponse(f);
    }
